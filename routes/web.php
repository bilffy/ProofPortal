...
Route::middleware(['auth', NoCacheHeaders::class])->group(function () {
    ...
    Route::middleware(['proofing_menu'])->group(function () use ($permissionCanProof, $permissionCanConfigProof, $permissionCanManageInvite, $permissionCanProofChange, $permissionCanBulkUpload, $permissionCanReport) {
        ...
        Route::group(['middleware' => ["permission:{$permissionCanManageInvite}", CheckUserRestriction::class]], function () {
            //Invitation
            Route::get('/franchise/access', [InvitationController::class, 'showInvitation'])->name('invitation.showInvitation');
            //Invitation - index
            Route::get('/proofing/access/index/{role}', [InvitationController::class, 'index'])->name('invitation.index');
            //Invitation - send
            Route::post('/proofing/access/send', [InvitationController::class, 'inviteSend'])->name('invitations.inviteSend');
        ...
        Route::group(['middleware' => ["permission:{$permissionCanConfigProof}", CheckUserRestriction::class, CheckJobSession::class]], function () {
            ...
            //Configure Job - Async Image Count (TNJ vs Portal thumbnail comparison, deferred off the main page load)
            Route::get('/franchise/config-job/image-count/{hash}', [JobConfigureController::class, 'imageCount'])->name('config-job.imageCount')->middleware('signed');
        ...
        Route::group(['middleware' => ["permission:{$permissionCanManageInvite}", CheckUserRestriction::class, CheckJobSession::class]], function () {
            ...
            //Invitation - single
            Route::get('/proofing/access/single/{role}/{hash}', [InvitationController::class, 'inviteSingle'])->name('invitations.inviteSingle')->middleware('signed');
            //Invitation - multiple
