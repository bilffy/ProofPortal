<div class="w-full">
    <h5 class="text-xl font-bold text-gray-900">Revoke Users</h5>
    <p class="pt-2 text-sm text-gray-600">Bulk-revoke user access for the selected season.</p>

    <div class="mt-5 flex flex-col sm:flex-row sm:items-end gap-4">
        <div class="flex flex-col gap-2 min-w-[16rem]" wire:ignore>
            <label for="revoke-users-season" class="text-sm font-semibold text-gray-700">Season</label>
            <select
                id="revoke-users-season"
        ...
        <div>
            <button
                type="button"
                onclick="window.runSeasonAction('revoke-users-season', 'revokeUsersForSeason')"
                wire:loading.attr="disabled"
                wire:target="revokeUsersForSeason"
            ...
            >
                <span wire:loading.remove wire:target="revokeUsersForSeason">Revoke Users</span>
                <span wire:loading wire:target="revokeUsersForSeason">Revoking...</span>
            </button>
        </div>
    </div>
    ...
    @if ($statusMessage)
        {{-- Hex styles: theme has no default green-*/red-* utilities in compiled CSS --}}
        <div
