...
<div class="relative">
    <div class="flex flex-row gap-4">
        <div class="w-[200px]">
            ...
            <x-form.select context="groups_year" :options="$yearOptions" class="mb-4" :disabled="$isSingleYear">Year</x-form.select>    
        ...
        @else
            ...
            <div class="w-full text-center">{{ $photographyMessages['no_jobs'][$level] }}</div>
        @endif
    </div>
</div>
...
<script type="module">
    ...
    function updateGridView(event) {
        const $classSelect = $('#select_groups_class');
        let classValues = $classSelect.val() || [];
        
        if (event && event.currentTarget && event.currentTarget.id === 'select_groups_class') {
